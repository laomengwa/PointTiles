	...
	match $'/root/根场景/主场景'.歌曲类型格式:
		0:
			if GlobalScript.游戏开始状态==true:
				判定偏移=(1-(20*(4/float($'/root/根场景/主场景'.物件生成每节节拍))*($"../../判定线".position[1]-0.5))/(60000000.0/float($'/root/根场景/主场景'.物件生成微秒每拍)))+GlobalScript.判定偏移
		1:
			if GlobalScript.游戏开始状态==true:
				判定偏移=(1-(20*GlobalScript.谱面基础节拍[谱面阶段]*($"../../判定线".position[1]-0.5))/GlobalScript.谱面每分钟节拍[谱面阶段])+GlobalScript.判定偏移
			#给长条添加着色器
			if self.has_node('模型/长条尾')==true:
				...
				长条头材质.albedo_texture=preload('res://mesh/长条头.png')
				长条腰材质.albedo_texture=preload('res://mesh/长条腰.png')
				长条尾材质.albedo_texture=preload('res://mesh/长条尾.png')
				...
				长条头材质.next_pass.shader=preload('res://shader/物件边框.gdshader')
				长条腰材质.next_pass.shader=preload('res://shader/物件边框.gdshader')
				长条尾材质.next_pass.shader=preload('res://shader/物件边框.gdshader')
				...
				长条头材质.next_pass.next_pass.shader=preload('res://shader/长块按下.gdshader')
				长条腰材质.next_pass.next_pass.shader=preload('res://shader/长块按下.gdshader')
				长条尾材质.next_pass.next_pass.shader=preload('res://shader/长块按下.gdshader')
...
func _process(delta):
		...
		match $'/root/根场景/主场景'.歌曲类型格式:
			1:
				...
				if self.has_node('模型/打击判定轨道')==true:
					...
					#判断出现时间
					if 狂戳轨道提示[音符摆放测试]-判定偏移<=$'/root/根场景/视角节点/背景音乐播放节点'.播放时间:
						if 狂戳节奏模式==true:
							var 粉块=狂戳打击显示.instantiate()
							粉块.position=Vector3(0,0.75,0)
							粉块.显示编号=狂戳轨道提示.size()-音符摆放测试
							粉块.音符出现时间 = 狂戳轨道提示[音符摆放测试]
							if 音符摆放测试%2==0:
								粉块.提示显示方向=true
							else:
								粉块.提示显示方向=false
							if 音符摆放测试<狂戳轨道提示.size()-1:
			...
			#终止声音播放
				for 声音数据集合循环 in 音符序列.size():
					if 音符消除状态==true||长条按下事件==true||self.has_node('模型/打击判定轨道')==true:
						if ((音符序列[声音数据集合循环].出现时间-GlobalScript.阶段时间位置[$/root/根场景/主场景.谱面阶段])*60/GlobalScript.谱面每分钟节拍[$/root/根场景/主场景.谱面阶段]/32)+$/root/根场景/主场景.谱面段落时间差+$/root/根场景/主场景.音频延迟<=$'/root/根场景/视角节点/背景音乐播放节点'.播放时间:
					...
					if (((音符序列[声音数据集合循环].出现时间+音符序列[声音数据集合循环].时长)-GlobalScript.阶段时间位置[谱面阶段])*60/GlobalScript.谱面每分钟节拍[谱面阶段]/32)+$"/root/根场景/主场景".谱面段落时间差+$"/root/根场景/主场景".音频延迟<=$'/root/根场景/视角节点/背景音乐播放节点'.播放时间:
	...
	pass
#func _exit_tree():
	#if $"/root/根场景/主场景/开始按键".visible==false&&GlobalScript.游戏开始状态==true:
		#self.free()
	#pass
...
func 音符消除():
	...
	#爆裂块
		...
		self.get_node('模型/触摸区域').hide()
	#黑块
	elif self.has_node('模型/长条尾')==false:
		音符消除状态=true
		var 黑块材质=ShaderMaterial.new()
		黑块材质.shader=preload('res://shader/黑块消除.gdshader')
		黑块材质.next_pass=ShaderMaterial.new()
...
func 漏击判定():
	if $"/root/根场景/主场景".自动模式==false:
		...
		self.queue_free()
	pass
func 点击事件(camera, event, 触摸点位置, normal, shape_idx):
	if self.has_node('模型/长条尾')==true&&音符消除状态==false	:
		#长条按下时的视觉印记
		if GlobalScript.游戏开始状态==true:
			self.get_node("模型/长条头").get_active_material(0).next_pass.next_pass.set_shader_parameter("time",((self.position[1]-触摸点位置[1])/-3)+2)
	if event is InputEventScreenTouch:
		...
		if self.has_node('模型/长条尾')==true:
			if 音符消除状态==false&&长条按下事件==false:
				...
				长条按下事件=true
			#松开则就判定为失误
			if event.pressed==false&&音符消除状态==false&&GlobalScript.游戏开始状态==true&&长条按下事件==true:
				$"../..".失误判定(false)
				音符消除状态=true
				#self.queue_free()
		else:
			#其他物件
			if event.pressed == true && GlobalScript.游戏开始状态==true&&音符消除状态==false:
...
func 长块腰触摸区域(camera, event, 触摸点位置, normal, shape_idx):
	if 长条按下事件==true&&音符消除状态==false:
		...
		if event is InputEventScreenTouch:
			if event.pressed == false && GlobalScript.游戏开始状态==true &&长条按下事件==true:
				self.音符消除()
				音符消除状态=true
				$"../..".失误判定(false)
	pass # Replace with function body.
...
func 打击判定(尾判:bool):
	...
	elif 位置判定+(GlobalScript.判定偏移/0.375) >= $"../../判定线".position[1]-4.5 && 位置判定+(GlobalScript.判定偏移/0.375) <= $"../../判定线".position[1]+4.5:
...
func 滑条按下(camera, event, position, normal, shape_idx):
	self.音符消除()
	pass # Replace with function body.

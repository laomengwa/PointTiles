...
var 声音字体文件路径:String="res://music/TimGM6mb.sf2"
var 音频设备编号:int=0
var 总音量数据:float=1.0
	...
	if 读取结果==OK:
		if 设置数据.has_section("音频"):
			...
			if 设置数据.has_section_key("音频","音频延迟"):
				GlobalScript.音频延迟=设置数据.get_value("音频","音频延迟")
...
func 保存设置数据(设置数据):
	...
	设置数据.set_value("音频","音频延迟",GlobalScript.音频延迟)
...
func 设置应用():
	...
	#音频延迟
	$'音频/其他/容器/音频延迟/滑块'.value=GlobalScript.音频延迟
	$'音频/其他/容器/音频延迟/数值'.text=var_to_str(int(GlobalScript.音频延迟))
	#演奏模式

...
func _ready():
	#传统谱面列表读取
	##代码逻辑混乱，未来会考虑重构
	if $'界面左列表/容器/旧式谱面列表/列表'.get_child_count() <= 1:
		#检测歌曲列表文件是否存在
		if FileAccess.file_exists("res://音乐/旧式谱面/表格配置/music_json")==true:
			$'界面左列表/容器/搜索栏/输入框'.editable=false
			$'界面左列表/容器/搜索栏/输入框'.placeholder_text="歌曲列表加载状态下不能搜索"
			$'界面左列表/容器/搜索栏/输入框'.tooltip_text="歌曲列表加载状态下不能搜索"
			添加指定传统歌曲(1)
			$"界面左列表/容器/翻页栏/当前页".text="1"
			#删除正在加载歌曲的标签节点
			$'界面左列表/容器/搜索栏/输入框'.editable=true
			$'界面左列表/容器/搜索栏/输入框'.placeholder_text="输入歌曲名以搜索……"
			$'界面左列表/容器/搜索栏/输入框'.tooltip_text="输入歌曲名以搜索……"
			$'界面左列表/容器/加载提示标签'.hide()
		else:
			...
			$'界面左列表/容器/旧式谱面列表/列表'.add_child(歌曲无文件提示)
			print("无文件")
	var 数码乐谱文件路径="res://音乐/二进制谱面/"
	for 文件循环 in DirAccess.get_files_at(数码乐谱文件路径).size():
		...
		$'界面左列表/容器/数码曲谱列表/列表'.add_child(传统歌曲卡片)
func _process(帧处理):
	#UI自适应
	#竖屏
	...
	#全横屏
		...
		pass
	pass
func 添加指定传统歌曲(起始页面:int,歌曲卡片数量:int=30,指定节点:Node=$'界面左列表/容器/旧式谱面列表/列表')->void:
	#删除旧的歌曲卡片
	歌曲加载状态=true
	$"界面左列表/容器/翻页栏".自定义歌曲网格节点=self
	...
	#用于添加“歌曲卡片数量“个翻页歌曲卡片
	var 循环次数:int=歌曲卡片数量
	$"界面左列表/容器/翻页栏".总页面=ceili((全局脚本.传统文件列表[1].size()-1)/歌曲卡片数量)+1
	if 起始页面*歌曲卡片数量>=全局脚本.传统文件列表[1].size()-1:
		if 歌曲卡片数量>=全局脚本.传统文件列表[1].size()-1:
			循环次数=全局脚本.传统文件列表[1].size()-1
		else:
			循环次数=(全局脚本.传统文件列表[1].size()-1)-(起始页面-1)*歌曲卡片数量
		pass
	$'界面左列表/容器/加载提示标签'.max_value=循环次数
	$'界面左列表/容器/加载提示标签'.show()
	#循环level文件
	for 循环 in 循环次数:
		await get_tree().create_timer(0.0).timeout
		$'界面左列表/容器/加载提示标签/进度'.text=var_to_str(循环+1)+"/"+var_to_str(循环次数)
		$'界面左列表/容器/加载提示标签'.value=循环+1
		#循环music_json文件
		for 对象循环 in 全局脚本.传统文件列表[0].size()-1:
			if 全局脚本.传统文件列表[1][(起始页面-1)*歌曲卡片数量+循环+1].Mid == 全局脚本.传统文件列表[0][对象循环].Mid:
				var 传统歌曲卡片=歌曲卡片场景.instantiate()
				传统歌曲卡片.get_node("容器/容器/歌曲名").text = 全局脚本.传统文件列表[0][对象循环].MusicJson
				传统歌曲卡片.get_node("容器/容器/艺术家").text = 全局脚本.传统文件列表[0][对象循环].Musician
				传统歌曲卡片.get_node("容器/卡片头/编号").text = var_to_str((起始页面-1)*歌曲卡片数量+循环+1)
				var 准备按钮 = Button.new()
				准备按钮.text = "准备"
				准备按钮.custom_minimum_size=Vector2(100,0)
				var 封面歌曲名=全局脚本.传统文件列表[0][对象循环].MusicJson
				var 封面艺术家=全局脚本.传统文件列表[0][对象循环].Musician
				var 临时谱面歌曲速度=全局脚本.传统文件列表[0]
				var 谱面歌曲速度循环=对象循环
				准备按钮.pressed.connect(歌曲卡片按钮按下.bind(封面歌曲名,封面艺术家,临时谱面歌曲速度,谱面歌曲速度循环,全局脚本.传统文件列表[0][对象循环].Acceleration))
				#设置卡片头纹理
				if 全局脚本.传统文件列表[1][(起始页面-1)*歌曲卡片数量+循环+1].MusicCard!="":
					var 卡片头纹理路径:String="res://音乐/旧式谱面/卡片头图片/"+全局脚本.传统文件列表[1][(起始页面-1)*歌曲卡片数量+循环+1].MusicCard+".png"
				...
				传统歌曲卡片.get_node("容器/容器/成绩段位/收藏").pressed.connect(歌曲收藏按钮按下.bind(传统歌曲卡片,准备按钮,封面歌曲名,封面艺术家,临时谱面歌曲速度,谱面歌曲速度循环,全局脚本.传统文件列表[0][对象循环].Acceleration))
				break
			pass
		pass
	$"界面左列表/容器/翻页栏".刷新状态()
	...
	pass
func 歌曲收藏按钮按下(组件,按钮,封面歌曲名,封面艺术家,临时谱面歌曲速度,谱面歌曲速度循环,竞技场加速度):
	if 组件.get_node("容器/容器/成绩段位/收藏").button_pressed==true:
		var 组件复制=组件.duplicate()
		var 按钮复制=按钮.duplicate(DUPLICATE_SIGNALS)
		按钮复制.pressed.connect(歌曲卡片按钮按下.bind(封面歌曲名,封面艺术家,临时谱面歌曲速度,谱面歌曲速度循环,竞技场加速度))
		组件复制.get_node("容器/容器/成绩段位").add_child(按钮复制)
		$'界面左列表/容器/收藏列表/列表'.add_child(组件复制)
	else:
		for 循环 in $'界面左列表/容器/收藏列表/列表'.get_child_count():
			if $'界面左列表/容器/收藏列表/列表'.get_child(循环).get_node('容器/卡片头/编号').text==组件.get_node('容器/卡片头/编号').text&&$'界面左列表/容器/收藏列表/列表'.get_child(循环).get_node('容器/容器/歌曲名').text==组件.get_node('容器/容器/歌曲名').text&&$'界面左列表/容器/收藏列表/列表'.get_child(循环).get_node('容器/容器/艺术家').text==组件.get_node('容器/容器/艺术家').text:
				$'界面左列表/容器/收藏列表/列表'.get_child(循环).queue_free()
	pass
func 数码曲谱卡片按下(文件路径,父文件夹路径):
	...
	#大型黑乐谱
		...
		通知.get_node("控件/内容").text="文件过大，无法读取"
		$'/root/根场景/根界面/通知栏'.add_child(通知)
	pass

func 歌曲卡片按钮按下(封面歌曲名,封面艺术家,临时谱面歌曲速度,谱面歌曲速度循环,竞技场加速度):
	...
	if $'/root/根场景/根界面/界面动画'.is_playing()==false:
		...
		match $'/root/根场景/主场景'.歌曲类型格式:
			...
			0:
				...
				for 事件块 in 全局脚本.数码乐谱文件数据.tracks[0].events:
...
func 恢复搜索状态():
	$界面左列表/容器/旧式谱面列表.hide()
	$界面左列表/容器/数码曲谱列表.hide()
	$界面左列表/容器/测试曲谱列表.hide()
	$界面左列表/容器/收藏列表.hide()
	#恢复搜索状态
	...
	for 菜单循环 in $'界面左列表/容器'.get_child_count()-1:
		...
		if $'界面左列表/容器'.get_child(菜单循环+1).visible==true:
			for 子节点循环 in get_node(菜单组建路径).get_child(0).get_child_count():
				...
				子节点.visible=true
	#检查歌曲收藏状态
	for 卡片循环 in $'界面左列表/容器/收藏列表/列表'.get_child_count():
		if $'界面左列表/容器/收藏列表/列表'.get_child(卡片循环).get_node('容器/容器/成绩段位/收藏').button_pressed==false:
			for 循环 in $'界面左列表/容器/旧式谱面列表/列表'.get_child_count():
				if $'界面左列表/容器/旧式谱面列表/列表'.get_child(循环).has_node('容器/卡片头/编号'):
					if $'界面左列表/容器/旧式谱面列表/列表'.get_child(循环).get_node('容器/卡片头/编号').text==$'界面左列表/容器/收藏列表/列表'.get_child(卡片循环).get_node('容器/卡片头/编号').text&&$'界面左列表/容器/旧式谱面列表/列表'.get_child(循环).get_node('容器/容器/歌曲名').text==$'界面左列表/容器/收藏列表/列表'.get_child(卡片循环).get_node('容器/容器/歌曲名').text&&$'界面左列表/容器/旧式谱面列表/列表'.get_child(循环).get_node('容器/容器/艺术家').text==$'界面左列表/容器/收藏列表/列表'.get_child(卡片循环).get_node('容器/容器/艺术家').text:
						$'界面左列表/容器/旧式谱面列表/列表'.get_child(循环).get_node('容器/容器/成绩段位/收藏').button_pressed=false
						$'界面左列表/容器/收藏列表/列表'.get_child(卡片循环).queue_free()
	pass # Replace with function body.
...
func 旧式歌曲列表() -> void:
	恢复搜索状态()
	$界面左列表/容器/旧式谱面列表.show()
	pass # Replace with function body.
func 正式歌曲列表() -> void:
	恢复搜索状态()
	$界面左列表/容器/数码曲谱列表.show()
	pass # Replace with function body.
func 测试歌曲列表() -> void:
	恢复搜索状态()
	$界面左列表/容器/测试曲谱列表.show()
	pass # Replace with function body.
func 收藏歌曲列表() -> void:
	恢复搜索状态()
	$界面左列表/容器/收藏列表.show()

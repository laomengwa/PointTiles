...
func _read_string( stream:StreamPeerBuffer, size:int ) -> String:
	var 字符数组:PackedByteArray=stream.get_partial_data( size )[1]
	#var 万国码字符紧凑数组:PackedByteArray=[]
	#var 万国码字符数组:Array=[]
	#for 循环 in 字符数组.size():
		#if 字符数组[循环]>127:
			#万国码字符紧凑数组.append(字符数组[循环])
			#if 万国码字符紧凑数组.size()==2:
				#万国码字符数组.append(GbkDecode.map_gbk_to_utf8(万国码字符紧凑数组[0]*万国码字符紧凑数组[1]))
				#万国码字符紧凑数组.clear()
	##万国码字符数组.append(GbkDecode.map_gbk_to_utf8(int(字符数组[2*循环])*int(字符数组[2*循环+1])))
	#万国码字符紧凑数组=GbkDecode.transform_utf8_ints_to_dynamic_length_bytes(万国码字符数组)
	#print(万国码字符紧凑数组.get_string_from_utf8( ))
	#万国码
	if 字符数组.get_string_from_utf8( ).find("�")==-1:
		return 字符数组.get_string_from_utf8( )
	#elif 字符数组.get_string_from_utf32( ).find("�")==-1:
		#return 字符数组.get_string_from_utf32( )
	#elif 字符数组.get_string_from_wchar().find("�")==-1:
		#return 字符数组.get_string_from_wchar()
	#elif 字符数组.get_string_from_utf16( ).find("�")==-1:
		#return 字符数组.get_string_from_utf16( )
	return 字符数组.get_string_from_ascii( )
